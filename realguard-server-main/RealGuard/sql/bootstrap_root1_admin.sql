-- 在「能执行 sudo mysql」的机器上运行一次（不需要知道 MySQL root 密码）：
--   sudo mysql < /path/to/RealGuard/sql/bootstrap_root1_admin.sql
--
-- 作用：创建库 system、全量表、应用账号 root1（与 utils.py 中 DB_CONFIG 一致）。
-- 密码须满足服务器 validate_password 策略（过弱会报 ERROR 1819）。
-- 连接 127.0.0.1 时需同时授权 127.0.0.1 与 localhost 两种 host。
...
DROP USER IF EXISTS 'root1'@'localhost';
DROP USER IF EXISTS 'root1'@'127.0.0.1';

CREATE USER 'root1'@'localhost' IDENTIFIED BY '<CHANGE_ME_STRONG_APP_DB_PASSWORD>';
CREATE USER 'root1'@'127.0.0.1' IDENTIFIED BY '<CHANGE_ME_STRONG_APP_DB_PASSWORD>';

GRANT ALL PRIVILEGES ON `system`.* TO 'root1'@'localhost';
GRANT ALL PRIVILEGES ON `system`.* TO 'root1'@'127.0.0.1';
