      ...
      <section className="result-feedback fallback-result-notice" aria-label="备用检测链路说明">
        <div className="swarm-upgrade">
          ...
          <div>
            ...
            <strong>{requestedSwarm ? "Swarm 模式未完成，当前展示备用证据链结果" : "快速检测未完成，当前展示备用证据链结果"}</strong>
            <p>当前报告来自备用视觉链路，已与原请求模式明确区分。</p>
          </div>
          {!requestedSwarm && (
            <button type="button" className="swarm-upgrade-button" disabled={upgradeBusy} onClick={onUpgrade}>
              {requiresLogin ? <LogIn size={16} /> : null}{requiresLogin ? "登录后使用 Swarm 模式" : "使用 Swarm 模式重新复核"}{requiresLogin ? null : <ArrowRight size={16} />}
            </button>
          )}
        </div>
      </section>
      ...
      <div className="result-feedback-main">
        <div className="result-feedback-copy">
          <span className={`result-mode-label ${isSwarm ? "is-swarm" : ""}`}>
            {isSwarm ? <Layers3 size={13} /> : null}{isSwarm ? "Swarm 模式" : "快速检测"}
          </span>
          <div>
      ...
      {feedback === -1 && (
        <div className="swarm-upgrade" role="status">
          ...
          <div>
            ...
            <p>{isSwarm ? "当前已经是 Swarm 模式结果，高风险场景建议结合原始来源与人工复核。" : "Swarm 模式会调度更多证据源，耗时更长，适合对当前结果存疑时使用。"}</p>
          </div>
          {!isSwarm && (
            <button type="button" className="swarm-upgrade-button" disabled={upgradeBusy} onClick={onUpgrade}>
              {requiresLogin ? <LogIn size={16} /> : null}
              {requiresLogin ? "登录后使用 Swarm 模式" : canReuseFile ? "使用 Swarm 模式重新复核" : "重新上传并使用 Swarm 模式"}
