...
type Filter = "all" | "fake" | "real" | "failed";
...
export default function DocumentBatchResult({ task }: Props) {
  ...
  const heading = summary?.verdictLabel
  ...
  const description = incompleteTerminal
    ...
    : terminal
      ? `共提取 ${task.discovered} 张图片，成功检测 ${task.succeeded} 张${task.failed ? `，${task.failed} 张失败` : ""}。`
    ...
    <section className="document-result" aria-label="文档图片检测结果">
      <header className={`document-result-summary ${incompleteTerminal ? "is-failed" : isFake ? "is-fake" : "is-real"}`}>
        ...
        <div className="document-result-score">
          <strong>{incompleteTerminal ? "—" : summary?.averageAiProbability == null ? `${task.progress}%` : percent(summary.averageAiProbability)}</strong>
          <span>{incompleteTerminal ? "未形成完整结论" : summary?.averageAiProbability == null ? "任务进度" : "平均 AI 风险"}</span>
        </div>
      </header>
      ...
      {filtered.length ? (
        <div className="document-asset-grid">
          {filtered.map((asset) => (
            <article key={`${asset.ordinal}-${asset.occurrenceIndex}`} className={`document-asset ${asset.status === "failed" ? "is-failed" : asset.verdict === "real" ? "is-real" : "is-fake"}`}>
              ...
              <div className="document-asset-copy">
                ...
                <strong>{asset.status === "failed" ? "检测失败" : asset.verdictLabel || (asset.verdict === "real" ? "真实图像" : "AI 生成图像")}</strong>
                <small>{asset.status === "failed" ? asset.error : `AI 风险 ${percent(asset.aiProbability)} · ${asset.width}×${asset.height}`}</small>
              </div>
            </article>
          ))}
        </div>
      ...
      {selected && (
        <div className="document-lightbox" role="dialog" aria-modal="true" aria-label={`${locationLabel(selected)}放大图`} onMouseDown={(event) => event.target === event.currentTarget && closeLightbox()}>
          <div ref={lightboxPanelRef} className="document-lightbox-panel">
            <header><div><strong>{locationLabel(selected)}</strong><span>{selected.verdictLabel} · AI 风险 {percent(selected.aiProbability)}</span></div><button ref={lightboxCloseRef} type="button" onClick={closeLightbox} aria-label="关闭放大视图"><X /></button></header>
