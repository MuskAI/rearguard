...
- 生产检测不再生成随机或模拟结论：模型不可用时返回明确错误；检测成功后最终结论固定为“真实图像”或“AI生成图像”。未通过独立校准门禁时仍按模型方向给出二元结论，但标记为低置信度并隐藏未校准概率。
...
- 结论语言：最终标签只使用“真实图像”和“AI生成图像”。只有通过独立签名校准门禁的模型，或严格验证的来源凭证/平台水印，才能发布已校准概率；未获决策授权时在解释中提示低置信度和复核建议，不再以中立状态替代最终标签。元数据缺失不能单独作为伪造证据。
